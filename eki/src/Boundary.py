...
class Boundary(object):
    ...
    def nearest(self, particles, bounds):
        ...
        newParticles = np.where(particles > upperBound, upperBound,
            newParticles)
        return newParticles
    ...
    def reflective(self, particles, bounds):
        ...
        while lower[0].size != 0 or upper[0].size != 0:
            if lower[0].size > 0:
                newParticles[lower] = 2 * lowerBound[lower[0]
                    ].T - newParticles[lower]
            if upper[0].size > 0:
                newParticles[upper] = 2 * upperBound[upper[0]
                    ].T - newParticles[upper]
    ...
    def random(self, particles, bounds):
        ...
        lower, upper = self.outOfBounds(particles, bounds)
        newParticles = particles
        newParticles[lower] = (np.array([(u - l) for u, l in zip(upperBound,
            lowerBound)]) * np.array([np.random.random_sample((particles.
            shape[0],))]).T + lowerBound)[lower[0]].T[0]
        newParticles[upper] = (np.array([(u - l) for u, l in zip(upperBound,
            lowerBound)]) * np.array([np.random.random_sample((particles.
            shape[0],))]).T + lowerBound)[upper[0]].T[0]
        return newParticles
    ...
    def periodic(self, particles, bounds):
        lowerBound, upperBound = np.array([bounds]).T
        lower, upper = self.outOfBounds(particles, bounds)
        bound_d = np.tile(np.abs(upperBound - lowerBound), (1, particles.
            shape[1]))
        ...
        if lower[0].size != 0 and lower[1].size != 0:
            newParticles[lower] = upperBound[lower] - np.mod(lowerBound[
                lower] - newParticles[lower], bound_d[lower])
        if upper[0].size != 0 and upper[1].size != 0:
            newParticles[upper] = lowerBound[upper] + np.mod(newParticles[
                upper] - upperBound[upper], bound_d[upper])
        return newParticles
