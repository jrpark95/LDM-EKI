...
#include "ldm_kernels_cram.cuh"  // Include for d_decay_constants declaration
#include "ldm_nuclides.cuh"  // Include for d_decay_constants
...
__global__ void move_part_by_wind_mpi(
    ...
    FlexPres* device_meteorological_flex_pres1){
        ...
        float x0 = p.x-xidx;
        float y0 = p.y-yidx;
        float z0 = (p.z-d_flex_hgt[zidx])/(d_flex_hgt[zidx+1]-d_flex_hgt[zidx]);
        float x1 = 1-x0;
        float y1 = 1-y0;
        float z1 = 1-z0;
        float t1 = 1-t0;
    ...
    p.conc = fminf(fmaxf(total, 0.0f), 1e20f);
