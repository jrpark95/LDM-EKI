...
bool EKIConfig::loadInputConfigFromFile(const std::string& input_config_filename) {
    std::cout << "[DEBUG] Loading EKI input_config from: " << input_config_filename << std::endl;
    
    std::ifstream file(input_config_filename);
    if (!file.is_open()) {
        std::cerr << "[ERROR] Cannot open input_config file: " << input_config_filename << std::endl;
        return false;
    }
    ...
    while (std::getline(file, line)) {
        ...
        // Parse sample_ctrl
        if (line.find("sample_ctrl:") != std::string::npos) {
            ...
            if (colon_pos != std::string::npos) {
                ...
                try {
                    sample_ctrl = std::stoi(value_str);
                    std::cout << "[DEBUG] Found sample_ctrl: " << sample_ctrl << std::endl;
                } catch (const std::exception& e) {
                    std::cerr << "[ERROR] Failed to parse sample_ctrl: " << value_str << std::endl;
                }
            }
        }
        ...
        if (line.find("iteration:") != std::string::npos) {
            ...
            if (colon_pos != std::string::npos) {
                ...
                try {
                    iteration = std::stoi(value_str);
                    std::cout << "[DEBUG] Found iteration: " << iteration << std::endl;
                } catch (const std::exception& e) {
                    std::cerr << "[ERROR] Failed to parse iteration: " << value_str << std::endl;
                }
            }
        }
    }
    ...
    std::cout << "[DEBUG] Input config loaded successfully:" << std::endl;
    std::cout << "[DEBUG]   Ensemble size (sample_ctrl): " << sample_ctrl << std::endl;
    std::cout << "[DEBUG]   Iterations: " << iteration << std::endl;
